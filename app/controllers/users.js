...
const forgetModel = require("../models/forget");
    ...
    if (!err) {
      ...
      forgetModel.create({ userid: results._id, code: text }, (err, done) => {
        if (!err) {
          ...
          var textHtml =
            '<h2>Password Reset Request</h2> <p>Dear Crimemap User,</p><p> We have received your request to reset your password. Please click the link below to complete the reset:</p> <p><a href="http://localhost:3000/api/user/forget/' +
            results._id +
            "/" +
            text +
            '"><button>Reset My Password</button></a></p>';
          textHtml +=
            "<p>If you need additional assistance, or you did not make this change, please contact bytenaija@gmail.com.</p>";
          textHtml +=
            "<p>if the button above does not work, copy the link below into your browser<br> http://localhost:3000/api/user/forget/" +
            results._id +
            "/" +
            text +
            "</p>";
      ...
      res.json({
        message:
          "We have sent you an email to reset your password user: " +
          req.params.userId
      });
    }
  });
    ...
    if (err) {
      ...
      if (result) {
        ...
        res.status(403).json({
          success: false,
          message:
            "The request code does not exist or has expired. Try resetting your password again"
        });
      }
    }
  });
        ...
        if (err) {
          ...
          if (result) {
            ...
            res.status(403).json({
              success: false,
              message:
                "We could not change your password, try resetting it again."
            });
          }
        }
      });
